...
var net = require('net');
var setFields = conf.getVal('geoObject');
var setServices = conf.getVal('services');
var setCommonOptions = conf.getVal('commonOptions');
var debug = require('debug')(conf.getVal('projectName') + ":app");
var GeoError = require('./helpers/geo-error');
...
GeoSearcher.init = function (options) {
    ...
    function findLocalFiles() {
        for (var serv in self.services) {
            if (!self.services.hasOwnProperty(serv)) {
                continue;
            }
...
GeoSearcher.filterIpv6 = function (ip) {
    if (!net.isIPv6(ip)) {
        return;
    }
    var self = this;
    for (var serviceId in self.services) {
        if (!self.services.hasOwnProperty(serviceId)) {
            continue;
        }
        var ipv6Enabled = conf.getVal('services:' + serviceId + ":v6");
        if (!ipv6Enabled) {
...
GeoSearcher.lookup = function (ip, callback) {
    ...
    if (this.optionsError !== null && this.optionsError !== undefined) {
        callback(new GeoError(this.optionsError), null);
    }
    ...
    function handleAccumulated() {
        ...
        for (var iter in keys) {
            if (!keys.hasOwnProperty(iter)) {
                continue;
            }
            result[keys[iter]] = null;
        }
        ...
        for (var iterFirst in accumulatedResult) {
            if (!accumulatedResult.hasOwnProperty(iterFirst)) {
                continue;
            }
        ...
        for (var i in keys) {
            ...
            for (var it in accumulatedResult) {
                if (!accumulatedResult.hasOwnProperty(it)) {
                    continue;
                }
    ...
    function setSearchedFields(fields) {
        for (var iter in fields) {
            if (!fields.hasOwnProperty(iter)) {
                continue;
            }
            if (setupFields.hasOwnProperty(iter)) {
    ...
    if (net.isIP(ip) === 0) {
        callback(new GeoError(ip + " is not IP"), null);
        return;
    }
    ...
    if (!Object.keys(this.services).length) {
        callback(new GeoError("No methods are allowed"), null);
    }
    ...
    for (var service in sorted) {
        if (!sorted.hasOwnProperty(service)) {
            continue;
        }
        //++cbStack;
        ...
        var cb = function (err, result) {
            ...
            if (cbStack <= 0 && !isDone) {
                ...
                callback(new GeoError("Geo data was not found"), null);
                //queue.kill();
                //queue.tasks = [];
            }
...
GeoSearcher.cleanServices = function () {
    for (var iter in this.services) {
        if (!this.services.hasOwnProperty(iter)) {
            continue;
        }
...
GeoSearcher.setPath = function (service, servName) {
    ...
    if (conf.getVal('services:' + servName + ":type") !== 'local') {
        return this;
    }
    var params = conf.getVal('services:' + servName + ":dbfile");
