...
var cidr = path.join(__dirname, "../../", conf.get('services:' + path.basename(path.dirname(__filename)) + ":cidrdb"));
var cities = path.join(__dirname, "../../", conf.get('services:' + path.basename(path.dirname(__filename)) + ":citiesdb"));
...
function validate(ip) {
    return new Promise((resolve, reject) => {
            var ipv6Enabled = conf.get('services:' + path.basename(path.dirname(__filename)) + ":v6");
            var check = net.isIP;
            if(!ipv6Enabled){
                check = net.isIPv4;
            }
