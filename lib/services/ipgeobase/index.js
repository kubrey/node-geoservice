...
var citiesBuffer = null;
var countriesBuffer = null;
...
function findCountry(ip, callback) {
    var instream = fs.createReadStream(cidr, {encoding: 'utf8'});
...
function findDetailedData(found, callback) {
    var instreamCity = fs.createReadStream(cities, {encoding: 'utf8'});
    var outstream = new stream;
    outstream.readable = true;
    outstream.writable = true;
    ...
    rlCity.on('close', function(){
        if(!foundOk){
            callback("City has not been found", null);
        }
    });
}
